...
for (const entry of readdirSync(".")) {
  if (
    entry.startsWith("game-warhammer40kdarktide-") &&
    entry.endsWith(".zip")
  ) {
...
const infoJson = JSON.parse(
  readFileSync(path.join(extDir, "info.json"), "utf8"),
);
...
if (process.platform === "win32") {
  ...
  execFileSync(
    ...
    [
      "-NoProfile",
      "-Command",
      `Compress-Archive -Path '${quote(path.join(distDir, "*"))}' -DestinationPath '${quote(path.resolve(zipName))}' -Force`,
    ],
    { stdio: "inherit" },
  );
} else {
  execFileSync("zip", ["-q", "-r", path.resolve(zipName), "."], {
    cwd: distDir,
    stdio: "inherit",
  });
}
